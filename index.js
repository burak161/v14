...
client.on("ready", async () => {
    ...
    console.log(`Bot şu şekilde giriş yaptı: ${client.user.tag}!`);
})
...
interaction.reply({content: "Çevrimiçi hız aktif!", ephemeral: true})
  }
...
.addComponents(
  ...
  .setURL("https://discord.gg/r2V5jUStTa")
)
...
client.on('interactionCreate', async interaction => {
  ...
  const queue = client.distube.getQueue(interaction);
     ...
     interaction.reply("Müziğin sesini başarıyla ayarlayın **"+volume+"**")
}
})
client.login(token)
